  ...
  if (O_DIRECT)
    {
      ...
      int __fuzzfix6 = 0 < fd_flags;
      if (__fuzzfix6)
        {
          int new_flags = (enable
                           ? (fd_flags | O_DIRECT)
                           : (fd_flags & ~O_DIRECT));
          int __fuzzfix7 = new_flags != fd_flags;
          if (__fuzzfix7)
...
#define PATTERNBUF_SIZE (PAGE_ALIGN_SLOP + FILLPATTERN_SIZE)
  ...
  int __fuzzfix9 = ! try_without_directio;
  if (__fuzzfix9)
    direct_mode (fd, true);
  ...
  while (true)
    {
      ...
      int __fuzzfix13 = type < 0;
      if (__fuzzfix13)
        randread (s, pbuf, lim);
      /* Loop to retry partial writes. */
      for (soff = 0; 1; soff += ssize)
        {
              ...
              if (! known (size) && (ssize == 0 || errno == ENOSPC))
                {
                  ...
                  int __fuzzfix16 = soff <= OFF_T_MAX - offset;
                  if (__fuzzfix16)
              ...
              else
                {
                  ...
                  int __fuzzfix17 = ! try_without_directio && errno == EINVAL;
                  if (__fuzzfix17)
                    {
      ...
      int __fuzzfix18 = OFF_T_MAX - offset < soff;
      if (__fuzzfix18)
        {
...
genpattern (int *dest, size_t num, struct randint_source *s)
{
  ...
  for (n = 0; 1; n++)
    {
      int __fuzzfix23 = n < num;
      if (!__fuzzfix23) break;
      int __fuzzfix24 = accum <= randpasses;
      if (__fuzzfix24)
        {
          accum += num - 1;
          dest[top++] = dest[n];
        ...
        struct Options const *flags)
{
  ...
  int __fuzzfix31 = fd_flags < 0;
  if (__fuzzfix31)
    {
      error (0, errno, _("%s: fcntl failed"), qname);
      return false;
    }
  int __fuzzfix32 = fd_flags & O_APPEND;
  if (__fuzzfix32)
    {
...
wipename (char *oldname, char const *qoldname, struct Options const *flags)
{
  ...
  int __fuzzfix34 = flags->remove_file == remove_wipesync;
  if (__fuzzfix34)
    dir_fd = open (dir, O_RDONLY | O_DIRECTORY | O_NOCTTY | O_NONBLOCK);

  int __fuzzfix35 = flags->verbose;
  if (__fuzzfix35)
    error (0, 0, _("%s: removing"), qoldname);

  while (1)
    {
      int __fuzzfix36 = (flags->remove_file != remove_unlink) && len;
      if (!__fuzzfix36) break;
          ...
          struct randint_source *s, struct Options const *flags)
{
  ...
  int __fuzzfix40 = ok && flags->remove_file;
  if (__fuzzfix40)
...
main (int argc, char **argv)
{
  ...
  int __fuzzfix42 = ! randint_source;
  if (__fuzzfix42)
